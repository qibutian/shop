...
public class QuestionbankFragment extends Fragment {
	...
	private void initView() {
		...
		reference_layout = (LinearLayout) mainV.findViewById(R.id.reference_layout);
	...
	private void getData() {
		...
		if (jsa!=null) {
			for (int i = 0; i < jsa.length(); i++) {
				...
				rb.setTextSize(14);
				rb.setButtonDrawable(getActivity().getResources().getDrawable(R.drawable.study_check_s));
				rb.setChecked(false);
	...
	private void submit() {
		...
		net.addParam("answer", answer.get(1).get(id));
		net.addParam("contentid", id);
		net.doPostInDialog(new NetTask(getActivity()) {
			...
			public void doInUI(Response response, Integer transfer) {
				...
				if (response.isSuccess()) {
					Toast.makeText(getActivity(), "已成功提交", Toast.LENGTH_SHORT).show();
					getActivity().finish();
